...
addRecord(item: GameData): Observable<GameData> {
  console.log(item);
...
updateRecord(item: GameData): Observable<GameData> {
  return this.http.put<any>('http://localhost:3000/data/' + item.id, item);
}

// Delete operation: Remove an item from the JSON file
deleteRecord(item: GameData): Observable<any> {
  return this.http.delete<any>('http://localhost:3000/data/' + item.id);
}
